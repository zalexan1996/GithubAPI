...
Function List-RepositoryIssues
{
    ...
    $QueryStrings = @("milestone=$milestone","state=$state","assignee=$assignee","creator=$creator","mentioned=$mentioned","labels=$labels","sort=$sort","direction=$direction","since=$since","per_page=$per_page","page=$page") | ? { $PSBoundParameters.ContainsKey($_) }
