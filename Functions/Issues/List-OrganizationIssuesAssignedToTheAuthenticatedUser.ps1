...
#>
Function List-OrganizationIssuesAssignedToTheAuthenticatedUser
{
    [CmdletBinding()]
    Param(
    ...
    $QueryStrings = @("filter=$filter","state=$state","labels=$labels","sort=$sort","direction=$direction","since=$since","per_page=$per_page","page=$page") | ? { $PSBoundParameters.ContainsKey($_) }

    
    if (![String]::IsNullOrEmpty($QueryStrings))
    {
        $FinalURL = "https://api.github.com/orgs/$org/issues?$($QueryStrings -join '&')"
    }
    else
    {
        $FinalURL = "https://api.github.com/orgs/$org/issues"
    }
